...
if len(sys.argv) == 3:
    ...
    for datai in datal:
        ...
        Edata = datai[0]      
        frags = datai[1]
        print "For E=",Edata," there are",frags," fragments in this data set"
        fragl = datai[2]
        for frag in fragl:
            Z  = frag[0]
            A  = frag[1]
        ...
        # write the output
        outl.sort(key=lambda x: x[1])
        writeout(str(Edata)+'_'+sys.argv[2],outl)

        print "The fragment info for incident energy",Edata," is in file",str(Edata)+'_'+sys.argv[2]
else:
    print "usage: yielddemo0.py inpfn outfn"
